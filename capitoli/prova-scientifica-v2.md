...
Inoltre, anche le scienze naturali sono in continua evoluzione.
Il loro obiettivo è creare un modello della realtà sempre più preciso.
Cercano, ma non pretendono, la perfezione.

Allo stesso modo, l'informatica forense si sforza di trovare le migliori modalità di acquisizione e valutazione dei dati informatici, ma data la complessità dei sistemi informatici, si può solo cercare di arrivare al miglior risultato possibile, anche se l'informatica di per sé ha natura affine alla matematica.

Passando all'uso concreto delle leggi e modelli elaborati dall'informatica forense nella sua fase di ricerca, si deve partire dall'assunto che i dati non possono essere presi *sic et simpliciter* per buoni.
Esistono numerose cause di modificazione dei dati, e che devono essere tenute in conto.

<!-- TODO: da qui -->

Studiare il funzionamento di un programma permette di determinare il "nesso causale" fra la "condotta" (l'uso doloso o colposo del programma) e l'evento (lo stato attuale dei dati informatici).
Se si conoscono gli effetti che il programma produce sui dati, è possibile ragionare al contrario, e valutare se lo stato dei dati possa essere stato causato da quel programma.
...
Il perito, essendo imparziale, cercherà di trovare tutte le possibili spiegazioni, ed indicherà la plausibilità di ognuna di esse, e gli argomenti a favore e contro ogni ipotesi di ricostruzione.
I consulenti tecnici, essendo di parte, si concentreranno solo sugli elementi che vanno a favore della strategia processuale della parte assistita, e sugli elementi che screditano la strategia dell'altra parte.
