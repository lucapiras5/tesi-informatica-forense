...
Il secondo tipo di caratteristiche impone l'uso di accortezze particolari nello sviluppo del software, che devono essere documentate dagli sviluppatori, ed eventualmente discusse dai tecnici, per dimostrare l'affidabilità del software.^[Ad esempio, se il software è scritto in un linguaggio *memory-unsafe* è utile usare Valgrind (<https://web.archive.org/web/20231113151236/http://valgrind.org/docs/manual/mc-manual.html>) per rilevare i bug relativi alla memoria. Se il software è scritto in un linguaggio *dynamically-typed*, è utile usare strumenti come MyPy (<https://mypy-lang.org/>) per controllare il corretto uso dei tipi&hellip; Più in generale, è utile seguire le linee-guida previste per il codice che sarà utilizzato in applicazioni critiche, come i sistemi aerospaziali, v. @Holzmann2006.]

È preferibile usare:

- Linguaggi *memory-safe* [Nei linguaggi *memory-safe*, la gestione della memoria è completamente automatica, v. @Pasini2019, 21] rispetto a linguaggi *memory-unsafe*.^[La gestione manuale della memoria nei linguaggi *memory-unsafe* è la causa della maggioranza dei bug. V. A. Gaynor, *Introduction to Memory Unsafety for VPs of Engineering*, 2019, <https://web.archive.org/web/20190812151808/https://alexgaynor.net/2019/aug/12/introduction-to-memory-unsafety-for-vps-of-engineering/>, e P. Kehrer, *Memory Unsafety in Apple's Operating Systems*, 2019, <https://web.archive.org/web/20190725163137/https://langui.sh/2019/07/23/apple-memory-safety/>.]
...
sono strumenti che verificano l'affidabilità e robustezza del codice, fornendo *input* casuali al programma, e osservando quali input causano un *crash* del programma.^[
Il *crash* è la situazione in cui l'esecuzione del programma termina in maniera inaspettata, e non prevista dal programmatore, perché una situazione di errore non è stato gestita correttamente.]
