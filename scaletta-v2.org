...
- Ad es., anche le norme di legge, che possono essere considerate un linguaggio di programmazione, non sono sempre chiare nel loro significato.
- Natural language processing -- branca dell'informatica che studia l'analisi del linguaggio naturale da parte dei computer.
- Language models -- intelligenze artificiali che generano testo in un linguaggio naturale.
- Chinese Room argument -- un'intelligenza artificiale non ha la capacità di "comprendere" cosa sta leggendo, può solo "ripetere" quanto ha imparato.
- Turing test -- un'intelligenza artificiale supera il test di Turing se le sue risposte sono indistinguibili da quelle di un essere umano.
- ChatGPT -- intelligenza artificiale
*** Linguaggio macchina
- I computer comprendono solo una serie ristretta di istruzioni estremamente concrete.
- Turing machine: modello teorico di computer, può eseguire solo alcune istruzioni.
- ISA -- Instruction Set Architecture, l'insieme di istruzioni che un processore può eseguire.
- CISC/RISC -- Complex/Reduced Instruction Set Computers, due filosofie diverse per definire una ISA.
- Differenza tra ISA, e implementazione concreta nei processori -- errata, Pentium FDIV bug
- ISA/processori più diffusi: x86, ARM.
- Estensioni alla ISA: FPU, SIMD
- Esempi di istruzioni in assembly, ed in binario
- Opcode e microcode
